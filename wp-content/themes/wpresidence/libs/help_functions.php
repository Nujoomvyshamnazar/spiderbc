...
function wpestate_return_all_fields($is_mandatory=0){
    ...
    $all_submission_fields=$all_mandatory_fields=array(
        ...
        'prop_category'                 =>  __('Prop Category Submit','wpestate'),
        'prop_action_category'          =>  __('Prop Action Category','wpestate'),
...
function wpestate_yelp_details($post_id) {
    ...
    foreach ( $yelp_terms as $key=>$term ) {
            ...
            if( isset($details->businesses) ){
                ...
                $start_lat  =   $details->region->center->latitude;
                $start_long =   $details->region->center->longitude;


                print '<div class="yelp_bussines_wrapper"><div class="yelp_icon"><i class="'.$category_icon.'"></i></div> <h4 class="yelp_category">'.$category_name.'</h4>';
                    foreach($category as $unit){
                    

                        print '<div class="yelp_unit">';
                            print '<h5 class="yelp_unit_name">'.$unit->name.'</h5>';
                            if(isset($unit->location->coordinate->latitude) && isset($unit->location->coordinate->longitude)){
                                 print ' <span class="yelp_unit_distance"> '.wpestate_calculate_distance_geo($unit->location->coordinate->latitude,$unit->location->coordinate->longitude,$start_lat,$start_long,$yelp_dist_measure).'</span>';
                            }
                            
                            $image_path=(string)$unit->rating;
                            $image_path= str_replace('.5', '_half', $image_path);
                            print '<img class="yelp_stars" src="'.get_template_directory_uri().'/img/yelp_small/small_'.$image_path.'.png" alt="'.$unit->name.'">';
